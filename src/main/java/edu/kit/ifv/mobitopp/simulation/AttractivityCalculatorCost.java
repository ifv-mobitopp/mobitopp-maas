...
public class AttractivityCalculatorCost
	...
	protected float getTravelCost(
		 ...
		 * TODO 
		 * 
		 * Keep car cost in future? We should check to change this to the cost depending on the concrete vehicle. 
		 * problem: vehicle is still unknown in destination choice!
		 */
	...
	protected float getParkingCost(
		...
		if ( mode == Mode.PEDESTRIAN 
				...
				|| mode == Mode.PASSENGER
				|| mode == Mode.RIDE_POOLING
				|| mode == Mode.AUTONOMOUS_TAXI
		) {
			return 0.0f;
		}
